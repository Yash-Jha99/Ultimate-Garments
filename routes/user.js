...
router.post("/", (req, res, next) => {
  ...
  db.query("select email from users where email=?", [email], (err, result) => {
    ...
    if (result[0])
      return res.status(400).json({ result: "User already registered" });
    else
      db.query(
        "insert into users (id,name,mobile_number,email,notify) values (?,?,?,?,?)",
        [id, name, mobileNumber, email, notify],
        (error, result) => {
...
router.put("/:id", (req, res) => {
  ...
  db.query(
    "update users set name=?,mobile_number=?,notify=?,gender=?,dob=? where id=?",
    [name, mobileNumber, notify, gender, dob, id],
    (err, result) => {
      if (err) console.log(err);
