        ...
        <div className={classes.mainContainer}>
            <div className={classes.box} >
                <Grid container rowGap={2} columnSpacing={2} >
                    <Grid item xs={12} className={classes.headingText}>SubCategory Interface</Grid>
                    <Grid item xs={4}>
                        <FormControl fullWidth>
                            <InputLabel id="select-label">Category</InputLabel>
                            <Select label="Category" labelId="select-label" value={category} onChange={(e) => setCategory(e.target.value)}>
                                {categories.map(category => <MenuItem key={category.value} value={category.value}>{category.text}</MenuItem>)}
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={8}>
                        <TextField fullWidth value={subCategoryName} variant="outlined" label="SubCategory Name" onChange={(e) => setSubCategoryName(e.target.value)} />
                    </Grid>
                    <Grid item xs={6}>
